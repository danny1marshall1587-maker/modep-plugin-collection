function (event, funcs) {
    ...
    var cabNames = [
        "1x12\" Oxford 12L6 Brownface (Stock)",
        "1x12\" Jensen P12Q",
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'channel') {
            ...
            pedal.find('#vibrolux-ch-btn').toggleClass('is-bright', isBright);
            pedal.find('#vibrolux-ch-txt').text(isBright ? "BRIGHT CH" : "NORMAL CH");
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#vibrolux-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#vibrolux-gate-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_spectral') {
            pedal.find('#vibrolux-spectral-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_defizz') {
            pedal.find('#vibrolux-defizz-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.vl touchstart.vl').on('mousedown.vl touchstart.vl', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.vl_drag touchmove.vl_drag', function (ev) {
            ...
            dial.off('dblclick.vl').on('dblclick.vl', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.vl_sync valuechange.vl_sync').on('change.vl_sync valuechange.vl_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.vibrolux-ch-btn, .vibrolux-mod-switch-pill, .vibrolux-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Channel Toggle
    pedal.find('#vibrolux-ch-btn').off('click.vl').on('click.vl', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#vibrolux-ch-txt').text(nextVal > 0.5 ? "BRIGHT CH" : "NORMAL CH");
    });

    // Speaker Cab Switcher
    pedal.find('#vibrolux-cab-btn').off('click.vl').on('click.vl', function (e) {
        e.stopPropagation();
