function (event, funcs) {
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'channel') {
            var isVib = fVal > 0.5;
            pedal.find('#magnatone-ch-btn').toggleClass('is-vib', isVib);
            pedal.find('#magnatone-ch-txt').text(isVib ? "VIBRATO CH" : "NORMAL CH");
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(1, parseInt(fVal) || 0));
            pedal.find('#magnatone-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#magnatone-gate-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_spectral') {
            pedal.find('#magnatone-spectral-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_defizz') {
            pedal.find('#magnatone-defizz-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.mag touchstart.mag').on('mousedown.mag touchstart.mag', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.mag_drag touchmove.mag_drag', function (ev) {
                ...
                $(window).on('mouseup.mag_drag touchend.mag_drag', function () {
                    $(window).off('.mag_drag');
                });
            });
            ...
            dial.off('dblclick.mag').on('dblclick.mag', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.mag_sync valuechange.mag_sync').on('change.mag_sync valuechange.mag_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.magnatone-ch-btn, .magnatone-mod-switch-pill, .magnatone-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Channel Toggle
    pedal.find('#magnatone-ch-btn').off('click.mag').on('click.mag', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#magnatone-ch-txt').text(nextVal > 0.5 ? "VIBRATO CH" : "NORMAL CH");
    });

    // Speaker Cab Switcher
    pedal.find('#magnatone-cab-btn').off('click.mag').on('click.mag', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#magnatone-cab-txt').text(cabNames[nextVal]);
    });

    
    // Smart Zero-Noise Gate Toggle Handler
    pedal.find('#magnatone-gate-btn').off('click.magnatone').on('click.magnatone', function (e) {
        e.stopPropagation();
    ...
    pedal.find('#magnatone-spectral-btn').off('click.magnatone').on('click.magnatone', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
    ...
    pedal.find('#magnatone-defizz-btn').off('click.magnatone').on('click.magnatone', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
