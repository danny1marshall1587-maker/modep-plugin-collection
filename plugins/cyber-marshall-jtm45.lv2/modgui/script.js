function (event, funcs) {
    ...
    var cabNames = [
        "4x12\" Celestion 20W Greenbacks (Stock)",
        "2x12\" Bluesbreaker Open-Back",
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'channel_link') {
            pedal.find('#jtm45-link-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#jtm45-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#jtm45-gate-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_spectral') {
            pedal.find('#jtm45-spectral-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_defizz') {
            pedal.find('#jtm45-defizz-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.jtm touchstart.jtm').on('mousedown.jtm touchstart.jtm', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.jtm_drag touchmove.jtm_drag', function (ev) {
                ...
                $(window).on('mouseup.jtm_drag touchend.jtm_drag', function () {
                    $(window).off('.jtm_drag');
                });
            });
            ...
            dial.off('dblclick.jtm').on('dblclick.jtm', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.jtm_sync valuechange.jtm_sync').on('change.jtm_sync valuechange.jtm_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.jtm45-mod-switch-pill, .jtm45-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Channel Link Toggle
    pedal.find('#jtm45-link-btn').off('click.jtm').on('click.jtm', function (e) {
        e.stopPropagation();
    ...
    pedal.find('#jtm45-cab-btn').off('click.jtm').on('click.jtm', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#jtm45-cab-txt').text(cabNames[nextVal]);
    });

    
    // Smart Zero-Noise Gate Toggle Handler
    pedal.find('#jtm45-gate-btn').off('click.jtm45').on('click.jtm45', function (e) {
        e.stopPropagation();
    ...
    pedal.find('#jtm45-spectral-btn').off('click.jtm45').on('click.jtm45', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
    ...
    pedal.find('#jtm45-defizz-btn').off('click.jtm45').on('click.jtm45', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
