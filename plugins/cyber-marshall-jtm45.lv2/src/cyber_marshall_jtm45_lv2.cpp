/*
 * Cyber Marshall JTM45 (1965) - British Bluesbreaker Tube Amplifier LV2 Plugin
 * Authentic analog circuit emulation of the 1965 Marshall JTM45,
 * featuring High Treble & Normal channels with input jumpering,
 * cathode follower driven tone stack, Drake transformer, 2x KT66 power section,
 * GZ34 rectifier sag, 4x12 Greenback Dynamic Speaker Stress, and Smart Zero-Floor Suppressor.
 */
...
#define PLUGIN_URI "http://cyber-audio.co.uk/plugins/cyber-marshall-jtm45"

enum PortIndex {
    PORT_AUDIO_IN       = 0,
    PORT_AUDIO_OUT      = 1,
    PORT_BYPASS         = 2,
    PORT_HIGH_TREB_VOL  = 3,
    PORT_NORMAL_VOL     = 4,
    PORT_CHANNEL_LINK   = 5,
    PORT_BASS           = 6,
    PORT_MIDDLE         = 7,
...
class CyberMarshallJTM45 {
public:
    CyberMarshallJTM45(double rate) : sampleRate(rate) {
        ...
        cabBbLow.setIdentity();
        cabBbMid.setIdentity();
        cabBbLp1.setIdentity();
        cabBbLp2.setIdentity();

        sagVoltage = 1.0f;
        gridCapNorm = gridCapHigh = gridCapCf = 0.0f;
...
initCabFilters();
    ...
    void initCabFilters() {
        // 4x12 Marshall 1960A Celestion 20W Greenbacks (Rich woody 80Hz resonance, chewy 2.4kHz bite, 5.2kHz cutoff)
        cabGbLow.setLowShelf(90.0f, 5.5f, 0.7f, sampleRate);
        cabGbMid.setPeaking(2400.0f, 4.2f, 1.3f, sampleRate);
        cabGbLp1.setLowPass(6800.0f, 0.707f, sampleRate);
        cabGbLp2.setLowPass(6800.0f, 0.707f, sampleRate);

        // 2x12 Bluesbreaker Open-Back (Warm open 75Hz low bloom, singing 2.8kHz chime)
        cabBbLow.setLowShelf(85.0f, 5.0f, 0.7f, sampleRate);
        cabBbMid.setPeaking(2800.0f, 3.8f, 1.4f, sampleRate);
        cabBbLp1.setLowPass(6800.0f, 0.707f, sampleRate);
        cabBbLp2.setLowPass(6800.0f, 0.707f, sampleRate);
    }
    ...
    inline float cathodeFollower(float in, float &gridBias) {
        ...
        float x = in - gridBias * 0.30f;
        float t = tanhf(x * 1.25f);
        return t - 0.10f * (t * t);
    }

    // 2x KT66 Tube Power Stage (Woody, thick, rich second-harmonic bloom)
    ...
    inline float processSpeaker(float in, int cabType, float driveVal) {
        ...
        if (cabType == 1) {
            s = cabBbLow.process(s);
            s = cabBbMid.process(s);
            s = cabBbLp1.process(s);
            s = cabBbLp2.process(s);
            return s * 1.02f;
        } else {
    ...
    void updateParams(float bass, float mid, float treb, float pres) {
        ...
        toneMid.setPeaking(750.0f, midDb, 1.2f, sampleRate);
        ...
        // High Treble bright cap (100pF across High Treble pot)
        brightFilter.setHighShelf(3400.0f, 6.0f, 0.7f, sampleRate);
    }
    ...
    , float pNoiseLevel = 5.0f) {
        ...
        float highGain = audioTaper(pHighVol, 3.8f);
        float normGain = audioTaper(pNormVol, 3.4f);
        ...
        for (uint32_t i = 0; i < n_samples; ++i) {
            ...
            if (isLinked) {
                sHigh = triode12AX7(s, gridCapHigh, 1.55f, 0.08f);
                sHigh = snubberHigh.lp(sHigh, 11500.0f, sampleRate);
                sHigh = brightFilter.process(sHigh);
                sHigh *= highGain;

                sNorm = triode12AX7(s, gridCapNorm, 1.45f, 0.08f);
                sNorm = snubberNorm.lp(sNorm, 11500.0f, sampleRate);
                sNorm *= normGain;
            ...
            // Long-Tail Pair Phase Inverter & 2x KT66 Power Stage
            float push = s * 3.4f;
            ...
            float currentDraw = fabsf(pOut) * 0.044f;
            ...
            // Dynamic 4x12 Greenback Speaker Simulation
            ...
            if (i == 0) {
                // printf("s=%.4f pOut=%.4f gateGain=%.4f gbCf=%.4f sag=%.4f\n", s, pOut, gateGain, gridCapCf, sagVoltage);
            }
output[i] = pOut;
        }
    }
...
private:
    ...
    Biquad cabBbLow, cabBbMid, cabBbLp1, cabBbLp2;
    OnePole snubberHigh, snubberNorm, snubberPower;
    OnePole dcBlockerPre, dcBlockerPost, couplingCapV1, couplingCapCf, couplingCapTone;

    float sagVoltage;
    float gridCapNorm, gridCapHigh, gridCapCf;
...
struct CyberMarshallJTM45LV2 {
    CyberMarshallJTM45* dsp;
    const float* audioIn;
    float* audioOut;
    const float* bypass;
    const float* highTrebVol;
    const float* normVol;
...
static LV2_Handle instantiate(const LV2_Descriptor* descriptor, double rate, const char* bundle_path, const LV2_Feature* const* features) {
    CyberMarshallJTM45LV2* handle = new CyberMarshallJTM45LV2();
    handle->dsp = new CyberMarshallJTM45(rate);
    return (LV2_Handle)handle;
}

static void connect_port(LV2_Handle instance, uint32_t port, void* data) {
    CyberMarshallJTM45LV2* h = (CyberMarshallJTM45LV2*)instance;
    switch ((PortIndex)port) {
        ...
        case PORT_HIGH_TREB_VOL:  h->highTrebVol = (const float*)data; break;
...
static void cleanup(LV2_Handle instance) { CyberMarshallJTM45LV2* h = (CyberMarshallJTM45LV2*)instance; if (h) delete h; }
