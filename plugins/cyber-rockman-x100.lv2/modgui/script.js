function (event, funcs) {
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'mode') {
            var m = Math.max(0, Math.min(3, parseInt(fVal) || 0));
            pedal.find('#rockman-mode-txt').text(modeNames[m]);
        } else if (symbol === 'chorus') {
            pedal.find('#rockman-chorus-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'bass_boost') {
            pedal.find('#rockman-bass-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#rockman-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#rockman-gate-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.rm touchstart.rm').on('mousedown.rm touchstart.rm', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.rm_drag touchmove.rm_drag', function (ev) {
                ...
                $(window).on('mouseup.rm_drag touchend.rm_drag', function () {
                    $(window).off('.rm_drag');
                });
            });
            ...
            dial.off('dblclick.rm').on('dblclick.rm', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.rm_sync valuechange.rm_sync').on('change.rm_sync valuechange.rm_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.rockman-mode-btn, .rockman-mod-switch-pill, .rockman-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Mode Switcher (Clean 1 -> Clean 2 -> Edge -> Dist)
    pedal.find('#rockman-mode-btn').off('click.rm').on('click.rm', function (e) {
        e.stopPropagation();
        var curVal = parseInt(pedal.find('.mod-knob-image[mod-port-symbol="mode"]').val()) || 0;
        var nextVal = (curVal + 1) % 4;
        sendPortValue('mode', nextVal);
        pedal.find('#rockman-mode-txt').text(modeNames[nextVal]);
    });

    // Chorus Toggle
    pedal.find('#rockman-chorus-btn').off('click.rm').on('click.rm', function (e) {
        e.stopPropagation();
        var curVal = parseFloat(pedal.find('.mod-knob-image[mod-port-symbol="chorus"]').val());
        if (isNaN(curVal)) curVal = 1.0;
        var nextVal = (curVal > 0.5) ? 0.0 : 1.0;
        sendPortValue('chorus', nextVal);
        $(this).toggleClass('active', nextVal > 0.5);
    });

    // Bass Boost Toggle
    pedal.find('#rockman-bass-btn').off('click.rm').on('click.rm', function (e) {
        e.stopPropagation();
        var curVal = parseFloat(pedal.find('.mod-knob-image[mod-port-symbol="bass_boost"]').val());
        if (isNaN(curVal)) curVal = 0.0;
        var nextVal = (curVal > 0.5) ? 0.0 : 1.0;
        sendPortValue('bass_boost', nextVal);
        $(this).toggleClass('active', nextVal > 0.5);
    });

    // Speaker Cab Switcher
    pedal.find('#rockman-cab-btn').off('click.rm').on('click.rm', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#rockman-cab-txt').text(cabNames[nextVal]);
    });

    // Zero-Noise Gate Toggle Handler
    pedal.find('#rockman-gate-btn').off('click.rm').on('click.rm', function (e) {
        e.stopPropagation();
