function (event, funcs) {
    ...
    var cabNames = [
        "Roland 2x12 Silver-Domes (Stock)",
        "Direct Out / Cab Bypass"
    ];

    var modeNames = ["OFF", "VIBRATO", "CHORUS"];
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'bright') {
            pedal.find('#jc120-sw-bright').toggleClass('on', fVal > 0.5);
        } else if (symbol === 'mod_mode') {
            var midx = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#jc120-mode-btn').removeClass('pos-0 pos-1 pos-2').addClass('pos-' + midx).text(modeNames[midx]);
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.jc touchstart.jc').on('mousedown.jc touchstart.jc', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.jc_drag touchmove.jc_drag', function (ev) {
                ...
                $(window).on('mouseup.jc_drag touchend.jc_drag', function () {
                    $(window).off('.jc_drag');
                });
            });
            ...
            dial.off('dblclick.jc').on('dblclick.jc', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.jc_sync valuechange.jc_sync').on('change.jc_sync valuechange.jc_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.jc120-bat-switch, .jc120-mode-btn, .jc120-deck-selector, .jc120-mod-switch-pill').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Bright Switch Toggle
    pedal.find('#jc120-sw-bright').off('click.jc').on('click.jc', function (e) {
        e.stopPropagation();
    ...
    // Modulation Mode Toggle (Off / Vibrato / Chorus)
    pedal.find('#jc120-mode-btn').off('click.jc').on('click.jc', function (e) {
        e.stopPropagation();
        var curVal = parseInt(pedal.find('.mod-knob-image[mod-port-symbol="mod_mode"]').val()) || 0;
        var nextVal = (curVal + 1) % 3;
        sendPortValue('mod_mode', nextVal);
        $(this).removeClass('pos-0 pos-1 pos-2').addClass('pos-' + nextVal).text(modeNames[nextVal]);
    });

    // Speaker Cab Switcher
    pedal.find('#jc120-cab-btn').off('click.jc').on('click.jc', function (e) {
        e.stopPropagation();
