/*
 * Cyber Screamer - LV2 Plugin
 * Ibanez TS808/TS9 Tube Screamer + Dynamic Fuzz Face Glassy Cleanup Volume Morphing
 *
 * Core Concept:
 *   Dynamic volume-driven circuit morphing:
 *     - Full Guitar Volume / Hard Pick Attack -> Tube Screamer Overdrive (JRC4558 soft clipping + signature 720Hz mid hump + active tone control + Fat mod)
 ...
 *     - Fat Mod: Extended bass response for thick single coils
 *     - Mix, Bypass
 */
...
#define PLUGIN_URI "http://cyber-audio.co.uk/plugins/cyber-screamer"
...
// Ibanez TS808/TS9 JRC4558 Soft Symmetrical Diode Feedback Clipping
static inline float ts9_drive_stage(float x, float drive) {
    float g = 1.0f + drive * 11.0f; // 1..12 gain
    float in_sig = x * g;

    // Symmetrical silicon diode clipping in feedback loop
    float clipped = tanhf(in_sig * 1.4f) * 0.85f;
    return clipped;
}

// Fuzz Face Germanium Dynamic Circuit
static inline float fuzzface_circuit(float x, float drive) {
    float g = 2.0f + drive * 6.5f;
    float gx = x * g;
    if (gx >= 0.0f) {
...
struct CyberScreamer {
    ...
    // TS9 720 Hz Mid-Hump & Active Tone Stack
    Biquad  ts_mid_hump;
    Biquad  ts_tone_shelf;
    Biquad  fat_mod_boost;
...
static LV2_Handle instantiate(const LV2_Descriptor*, double rate,
    ...
    // Iconic TS9 720 Hz pre-emphasis mid hump (+5.5 dB at 720 Hz, Q=1.1)
    p->ts_mid_hump.peaking(720.0f, 5.5f, 1.1f, (float)p->sample_rate);

    return (LV2_Handle)p;
}

static void connect_port(LV2_Handle handle, uint32_t port, void* data) {
    auto* p = (CyberScreamer*)handle;
    switch (port) {
        ...
        case PORT_FAT_MOD:   p->p_fat_mod  = (const float*)data; break;
...
static void activate(LV2_Handle handle) {
    ...
    p->jfet_sag.reset();
    p->ts_mid_hump.reset();
    p->ts_tone_shelf.reset();
    p->fat_mod_boost.reset();
    p->treble_sparkle_hp.reset();
    p->detector.reset();

    p->smooth_drive   = 0.45f;
    p->smooth_tone    = 0.50f;
...
static void run(LV2_Handle handle, uint32_t n_samples) {
    ...
    // Read control ports
    float t_drive   = p->p_drive   ? std::max(0.0f, std::min(100.0f, *p->p_drive)) / 100.0f : 0.45f;
    ...
    bool  fat_mod   = p->p_fat_mod ? (*p->p_fat_mod > 0.5f) : false;
    float t_mix     = p->p_mix     ? std::max(0.0f, std::min(100.0f, *p->p_mix)) / 100.0f : 1.00f;

    // TS9 Active Tone Control (3.2 kHz active low-pass / shelf sweep, -12 dB .. +10 dB)
    float tone_gain = (t_tone - 0.5f) * 22.0f;
    if (fabsf(tone_gain - p->prev_tone_gain) > 0.05f) {
        p->ts_tone_shelf.highShelf(3200.0f, tone_gain, (float)p->sample_rate);
        p->prev_tone_gain = tone_gain;
    }

    // Fat Mod: +4.5 dB low-end boost at 150 Hz
    if (fat_mod) {
        p->fat_mod_boost.lowShelf(150.0f, 4.5f, (float)p->sample_rate);
    }
    ...
    for (uint32_t i = 0; i < n_samples; ++i) {
        ...
        float x = raw_in;
        if (fat_mod) {
            x = p->fat_mod_boost.process(x);
        }

        // TS9 input high-pass 720 Hz pre-emphasis
        x = p->hp_in.hp(x, 100.0f, (float)p->sample_rate);
        x = p->lp_ts.lp(x, 7500.0f, (float)p->sample_rate);

        float sag = p->jfet_sag.lp(fabsf(x), 30.0f, (float)p->sample_rate);
        x *= (1.0f / (1.0f + sag * 0.75f));
        ...
        float morph = p->smooth_morph; // 0 = Glassy Clean, 1 = Tube Screamer

        // ─── Circuit Path A: Tube Screamer Overdrive ───
        float x_ts = ts9_drive_stage(x, p->smooth_drive);
        x_ts = p->ts_mid_hump.process(x_ts);
        x_ts = p->ts_tone_shelf.process(x_ts);
        ...
        float x_blended = morph * x_ts + (1.0f - morph) * x_fuzz_path;
