function (event, funcs) {
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'master_link') {
            pedal.find('#ac30pa-link-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#ac30pa-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#ac30pa-gate-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.pa touchstart.pa').on('mousedown.pa touchstart.pa', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.pa_drag touchmove.pa_drag', function (ev) {
                ...
                $(window).on('mouseup.pa_drag touchend.pa_drag', function () {
                    $(window).off('.pa_drag');
                });
            });
            ...
            dial.off('dblclick.pa').on('dblclick.pa', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.pa_sync valuechange.pa_sync').on('change.pa_sync valuechange.pa_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.ac30pa-mod-switch-pill, .ac30pa-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Master Link Toggle
    pedal.find('#ac30pa-link-btn').off('click.pa').on('click.pa', function (e) {
        e.stopPropagation();
        var curVal = parseFloat(pedal.find('.mod-knob-image[mod-port-symbol="master_link"]').val()) || 0;
        var nextVal = (curVal > 0.5) ? 0.0 : 1.0;
        sendPortValue('master_link', nextVal);
        $(this).toggleClass('active', nextVal > 0.5);
    });

    // Speaker Cab Switcher
    pedal.find('#ac30pa-cab-btn').off('click.pa').on('click.pa', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#ac30pa-cab-txt').text(cabNames[nextVal]);
    });

    // Zero-Noise Gate Toggle Handler
    pedal.find('#ac30pa-gate-btn').off('click.pa').on('click.pa', function (e) {
        e.stopPropagation();
