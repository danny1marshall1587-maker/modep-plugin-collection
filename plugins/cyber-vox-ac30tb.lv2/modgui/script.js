function (event, funcs) {
    ...
    var cabNames = [
        "2x12\" Celestion Alnico Blue (Stock)",
    ...
    function applyPortValue(symbol, value) {
        ...
        if (symbol === 'channel_link') {
            pedal.find('#ac30tb-link-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'speaker_cab') {
            var sc = Math.max(0, Math.min(2, parseInt(fVal) || 0));
            pedal.find('#ac30tb-cab-txt').text(cabNames[sc]);
        } else if (symbol === 'noise_gate') {
            pedal.find('#ac30tb-gate-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_spectral') {
            pedal.find('#ac30tb-spectral-btn').toggleClass('active', fVal > 0.5);
        } else if (symbol === 'noise_defizz') {
            pedal.find('#ac30tb-defizz-btn').toggleClass('active', fVal > 0.5);
        }
    }
    ...
    function initKnobs() {
        ...
        dials.each(function () {
            ...
            dial.off('mousedown.ac touchstart.ac').on('mousedown.ac touchstart.ac', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                $(window).on('mousemove.ac_drag touchmove.ac_drag', function (ev) {
                ...
                $(window).on('mouseup.ac_drag touchend.ac_drag', function () {
                    $(window).off('.ac_drag');
                });
            });
            ...
            dial.off('dblclick.ac').on('dblclick.ac', function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    pedal.find('.mod-knob-image').off('change.ac_sync valuechange.ac_sync').on('change.ac_sync valuechange.ac_sync', function () {
        var sym = $(this).attr('mod-port-symbol');
        var val = $(this).val();
        if (sym && typeof val !== 'undefined') applyPortValue(sym, val);
    });

    pedal.find('.ac30tb-mod-switch-pill, .ac30tb-deck-selector').on('mousedown touchstart pointerdown', function (e) {
        e.stopPropagation();
    });

    // Channel Link Toggle
    pedal.find('#ac30tb-link-btn').off('click.ac').on('click.ac', function (e) {
        e.stopPropagation();
    ...
    pedal.find('#ac30tb-cab-btn').off('click.ac').on('click.ac', function (e) {
        e.stopPropagation();
        ...
        pedal.find('#ac30tb-cab-txt').text(cabNames[nextVal]);
    });

    
    // Smart Zero-Noise Gate Toggle Handler
    pedal.find('#ac30tb-gate-btn').off('click.ac30tb').on('click.ac30tb', function (e) {
        e.stopPropagation();
    ...
    pedal.find('#ac30tb-spectral-btn').off('click.ac30tb').on('click.ac30tb', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
    ...
    pedal.find('#ac30tb-defizz-btn').off('click.ac30tb').on('click.ac30tb', function (e) {
        e.stopPropagation();
        var isAct = $(this).hasClass('active');
        var nextVal = isAct ? 0.0 : 1.0;
        $(this).toggleClass('active', nextVal > 0.5);
